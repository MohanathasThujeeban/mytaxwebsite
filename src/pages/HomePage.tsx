...
import { ShieldCheck, Zap, Users, Rocket, FolderOpen, AlertTriangle, ClipboardList, FileText, Banknote, Files } from 'lucide-react';
...
const ABOUT_CARDS: { icon: React.ReactNode; titleKey: string; descKey: string }[] = [
  { icon: <ShieldCheck size={28} />, titleKey: 'secureTitle', descKey: 'secureDesc' },
  { icon: <Zap size={28} />, titleKey: 'fastTitle', descKey: 'fastDesc' },
  { icon: <Users size={28} />, titleKey: 'supportTitle', descKey: 'supportDesc' },
];
    ...
    <main className="home-page">
      ...
      <section className="hero">
        ...
        <div className="hero__actions">
          <button className="btn-primary" onClick={() => navigate('/verify')}>
            <Rocket size={15} style={{ marginRight: 6, verticalAlign: 'middle' }} />{s['getStarted']}
          </button>
      ...
      <section id="services" className="home-section">
        <div className="section-wrapper">
          <p className="section-label reveal reveal-d1">{s['taxServicesTitle']}</p>
          <h2 className="section-heading reveal reveal-d2">{s['myTaxServices']}</h2>
          <div className="section-heading-divider" />

          <div className="services-grid">
            {SERVICES.map((svc, index) => (
              <div key={svc.code} className={`service-card reveal-d${(index % 6) + 1}`}>
                <div className="service-card__code">{svc.code}</div>
                <div className="service-card__body">
                  <div className="service-card__label">
                    {s[svc.labelKey]?.replace(/^M\d+ - /, '') ?? s[svc.labelKey]}
                  </div>
                  {svc.paid !== null && (
                    <span className={`service-card__tag ${svc.paid ? 'tag-paid' : 'tag-free'}`}>
                      {svc.paid ? 'Paid' : 'Free'}
                    </span>
                  )}
                </div>
      ...
      <section id="about" className="home-section home-section--alt">
        <div className="section-wrapper">
          ...
          <h2 className="section-heading reveal reveal-d2">{s['aboutTitle']}</h2>
          <div className="section-heading-divider" />
          ...
          <div className="about-cards">
            {ABOUT_CARDS.map((card, i) => (
              <div key={card.titleKey} className={`about-card reveal-d${i + 1}`}>
      ...
      <section id="guide" className="home-section">
        <div className="section-wrapper">
          ...
          <h2 className="section-heading reveal reveal-d2">{s['guideTitle']}</h2>
          <div className="section-heading-divider" />
          ...
          >
            {[
              ...
              { icon: <AlertTriangle size={20} />, key: 'taxFillingImportance' },
      ...
      <section
        ...
        style={{
          padding: '80px 40px',
          background: 'radial-gradient(ellipse at 50% 50%, rgba(100,100,100,0.07) 0%, transparent 70%)',
          textAlign: 'center',
        }}
      >
        <h2
