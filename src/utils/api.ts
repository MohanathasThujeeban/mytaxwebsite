...
export const api = {
  requestOtp: (payload: { nic: string; phone?: string; email?: string }) =>
    request<{ message: string; channel: string; expiresAt: string }>("/auth/otp", payload),

  verifyOtp: (payload: { nic: string; code: string }) =>
  ...
  login: (payload: { nic: string; password: string }) =>
    request<{ message: string; token: string; user: Record<string, unknown> }>("/auth/login", payload),
};
